...
public class Autobus {
    ...
    private BusDriver1 driver; // агрегация ("мягкая" связь)
    private Autopilot1 autopilot1; // композиция (жесткая / не разрывная связь)
    private final int capacity;

    private final autopark.Passenger[] passengers;
    private int countPassenger;

    public Autobus(BusDriver1 driver, int capacity) {
        this.driver = driver;
        driver.setAutobus(this);
        this.capacity = capacity;
        this.autopilot1 = new Autopilot1("AP-gpt6");
        this.id = counter++;
        this.passengers = new autopark.Passenger[capacity];
    }

    public boolean takePassenger(autopark.Passenger passenger) {
    ...
    private int isPassengerInBus(autopark.Passenger passenger) {
    ...
    public boolean dropOfPassenger(autopark.Passenger passenger) {
        if (passenger == null) return false;
        /*
        ...
        autopark.Passenger tempPassenger = passengers[index];
    ...
    public String getPassengersList() {
        ...
        for (int i = 0; i < passengers.length; i++) {
            if (passengers[i] != null) {
                ...
                autopark.Passenger temp = passengers[i];
                sb.append("autopark.Passenger: {id: ").append(temp.getId()).append("; name: ");
    ...
    public String toString() {
        StringBuilder sb = new StringBuilder("autopark.Autobus: {id: ");
        sb.append(id).append("; capacity:").append(capacity);
        sb.append("; driver: ").append(driver.toString());
        sb.append("; autopilot: ").append(autopilot1.toString())
                .append(" }");

        return sb.toString();

//        return "autopark.Autobus: {id: " + id +  "; capacity:" + capacity +
...
//                "; autopilot: " + autopilot.toString() + " }" ;
    ...
    public BusDriver1 getDriver() {
        return driver;
    }

    public void setDriver(BusDriver1 driver) {
    ...
    public Autopilot1 getAutopilot() {
        return autopilot1;
    }

    public void updateAutopilotSoftware(String softVersion) {
        this.autopilot1.setSoftwareVersion(softVersion);
    }
}